...
class GPTInferenceDataset:
  def __init__(self, args):
    self.args = args
    self.batch_size = args['dataset']['batch_size'] 

    dataset_path = os.path.join(args['dataset']['corpus']['root'], args['dataset']['corpus']['inference_path'])
    sentences = []
    ...
    embedding_path = os.path.join(args['dataset']['embeddings']['root'], args['dataset']['embeddings']['inference_path'])
    hidden_rep_layer = args['model']['model_layer']
    pretrained_model_name =  args['model']['backbone_model']
    inf_keys = args['dataset']['keys']['inference']
    self.embeddings_lst, self.untokenized_words = self.generate_subword_embeddings_and_untokenized_words_from_hdf5(sentences, embedding_path, hidden_rep_layer, pretrained_model_name, keys=inf_keys)
    self.inference_dataset = GPTInferenceIterator(self.embeddings_lst, self.untokenized_words)
    

      
  def merge_multiple_subwords(self, subword, is_beginning_subword):
  ...
  def match_tokenized_to_untokenized(self, tokenized_sent):
    ...
    Args:
      ...
      untokenized_sent: a string, no subword tok.
    Returns:
    ...
    while untokenized_sent_index < len(tokenized_sent):
      current_word = []
      while tokenized_sent_index + 1 < len(tokenized_sent) and not tokenized_sent[tokenized_sent_index + 1].startswith('Ġ'):
        mapping[untokenized_sent_index].append(tokenized_sent_index)
        current_word.append(tokenized_sent[tokenized_sent_index])
        tokenized_sent_index += 1
      mapping[untokenized_sent_index].append(tokenized_sent_index)
      current_word.append(tokenized_sent[tokenized_sent_index].replace('Ġ', ''))
      words.append(current_word)
      untokenized_sent_index = tokenized_sent_index + 1
      tokenized_sent_index += 1

    sorted_indices = sorted([key for key in mapping.keys()])
    return mapping, sorted_indices, words



  def generate_subword_embeddings_and_untokenized_words_from_hdf5(self, sentences, filepath, elmo_layer, model_name, skip_lines=False, keys=None):
    ...
    running_index = 0
    last_index = 0
    all_untokenized_words = []
    # print([observation.sentence for observation in observations])
    for key in keys:
      key_indices = [index.rsplit('-', 1)[-1]for index in indices if index.startswith(key)]
      ...
      for index in tqdm(sorted([int(x) for x in key_indices]), desc='[aligning {} embeddings]'.format(key)):
        if skip_lines and index in self.obvs_to_skip:
          offset += 1
          continue
        sentence = sentences[index-offset + running_index]

        hf_key = (key + '-' + str(index)) if key else str(index)
        feature_stack = hf[hf_key]
        single_layer_features = feature_stack[elmo_layer]
        tokenized_sent = subword_tokenizer.tokenize(sentence.strip())
        new_tokenized_sent = []
        old_tokenized_sent = tokenized_sent.copy()
        for idx, subword in enumerate(tokenized_sent):
          if "Ġ" in subword:
            split_tokens = self.merge_multiple_subwords(subword, idx==0)
            new_tokenized_sent.extend(split_tokens)
          else:
            new_tokenized_sent.append(subword)
        tokenized_sent = new_tokenized_sent
        untok_tok_mapping, first_word_positions, untokenized_words = self.match_tokenized_to_untokenized(tokenized_sent)
        all_untokenized_words.append(untokenized_words)

        assert single_layer_features.shape[0] == len(tokenized_sent)

        single_layer_features = torch.tensor([np.mean(single_layer_features[untok_tok_mapping[i][0]:untok_tok_mapping[i][-1]+1,:], axis=0) for i in first_word_positions])
        assert single_layer_features.shape[0] == len(first_word_positions)
        single_layer_features_list.append(single_layer_features)
        last_index = index
      
      running_index += last_index + 1
    print("single layer features list is", len(single_layer_features_list))
    return single_layer_features_list, all_untokenized_words
  ...
  def custom_pad(self, batch_items):
    ...
    '''
    seqs = [item[0].to(device=self.args['device']) for item in batch_items]
    words = [item[1] for item in batch_items]
    lengths = torch.tensor([len(x) for x in seqs], device=self.args['device'])
    seqs = nn.utils.rnn.pad_sequence(seqs, batch_first=True)
    return seqs, lengths, words
  ...
  def get_inference_dataloader(self):
    ...
    """
    return DataLoader(self.inference_dataset, batch_size=self.batch_size, collate_fn=self.custom_pad, shuffle=False)
...
class SimpleDataset:
  ...
  def __init__(self, args, task, vocab={}):
    self.args = args
    self.use_translit = args['dataset']['corpus']['use_translit']
    self.batch_size = args['dataset']['batch_size']
  ...
  def load_conll_dataset(self, filepath, skip_lines=False, key=""):
    ...
    for buf in self.generate_lines_for_sent(lines, skip_lines):
      ...
      if self.use_translit:
        match = re.search(r'Translit=([^|]+)', observation.extra_info)
        if match:
          translit_value = match.group(1)
          observation.sentence = translit_value
        else:
          raise Exception('No transliteration!')
...
class SubwordDataset(SimpleDataset):
  ...
  @staticmethod
  def match_tokenized_to_untokenized_gpt(tokenized_sent, untokenized_sent):
    """Aligns tokenized and untokenized sentence for GPT/LLaMA-style tokenization

    Computes an alignment between the un-subword-tokenized 
    and subword-tokenized sentences.

    Args:
        tokenized_sent: List of strings describing a subword-tokenized sentence
        untokenized_sent: List of strings describing a sentence, no subword tok.
    
    Returns:
        A dictionary mapping each untokenized sentence index 
    ...
    while (untokenized_sent_index < len(untokenized_sent) and tokenized_sent_index < len(tokenized_sent)):
      while (tokenized_sent_index + 1 < len(tokenized_sent)) and not tokenized_sent[tokenized_sent_index + 1].startswith('Ġ'):
        mapping[untokenized_sent_index].append(tokenized_sent_index)
        tokenized_sent_index += 1
      if tokenized_sent_index < len(tokenized_sent):
        mapping[untokenized_sent_index].append(tokenized_sent_index)
        untokenized_sent_index += 1
        tokenized_sent_index += 1
    if untokenized_sent_index == len(untokenized_sent)-1 and mapping[untokenized_sent_index][-1] != tokenized_sent_index:
      mapping[untokenized_sent_index].append(tokenized_sent_index)
  ...
  def generate_subword_embeddings_from_hdf5(self, observations, filepath, elmo_layer, subword_tokenizer=None, skip_lines=False, keys=None):
    ...
    for key in keys:
      ...
      for index in tqdm(sorted([int(x) for x in key_indices]), desc='[aligning {} embeddings]'.format(key)):
        ...
        feature_stack = hf[hf_key]
        single_layer_features = feature_stack[elmo_layer]
        tokenized_sent = subword_tokenizer.tokenize('[CLS] ' + joiner.join(observation.sentence) + ' [SEP]')
...
class GPTDataset(SubwordDataset):
  """Dataloader for conllx files and pre-computed GPT-like model embeddings.
  ...
  def generate_subword_embeddings_from_hdf5(self, observations, filepath, gpt_layer, subword_tokenizer=None, skip_lines=False, keys=None):
    '''Reads pre-computed subword embeddings from hdf5-formatted file.
    ...
    if subword_tokenizer == None:
      from transformers import AutoTokenizer
      model_name = self.args['model']['backbone_model']
      subword_tokenizer = AutoTokenizer.from_pretrained(model_name)
    hf = h5py.File(filepath, 'r')
    indices = list(hf.keys())
    single_layer_features_list = []
    ...
    for key in keys:
      ...
      for index in tqdm(sorted([int(x) for x in key_indices]), desc='[aligning {} embeddings]'.format(key)):
        ...
        hf_key = (key + '-' + str(index)) if key else str(index)
        feature_stack = hf[hf_key]
        single_layer_features = feature_stack[gpt_layer]
        tokenized_sent = subword_tokenizer.tokenize(joiner.join(observation.sentence))
        new_tokenized_sent = []
        old_tokenized_sent = tokenized_sent.copy()
        for idx, subword in enumerate(tokenized_sent):
          ...
          if "Ġ" in subword:
            ...
            new_tokenized_sent.append(subword)
        tokenized_sent = new_tokenized_sent
        # Split some words cos some words are merged somehow in tokenizer
        untokenized_sent = observation.sentence
        untok_tok_mapping = self.match_tokenized_to_untokenized_gpt(tokenized_sent, untokenized_sent)
        # print(single_layer_features.shape[0], len(tokenized_sent))
        # print(observation.sentence, observation.morph)
        assert single_layer_features.shape[0] == len(tokenized_sent)
        single_layer_features = torch.tensor([np.mean(single_layer_features[untok_tok_mapping[i][0]:untok_tok_mapping[i][-1]+1,:], axis=0) for i in range(len(untokenized_sent))])
  ...
  def optionally_add_embeddings(self, observations, pretrained_embeddings_path, skip_lines=False, keys=None):
    """Adds pre-computed GPT embeddings from disk to Observations."""
    layer_index = self.args['model']['model_layer']
    print('Loading GPT Pretrained Embeddings from {}; using layer {}'.format(pretrained_embeddings_path, layer_index))
    embeddings = self.generate_subword_embeddings_from_hdf5(observations, pretrained_embeddings_path, layer_index, skip_lines=skip_lines, keys=keys)
    observations = self.add_embeddings_to_observations(observations, embeddings)
    return observations
...
class GPTInferenceIterator(Dataset):
  def __init__(self, hidden_reps, untokenized_words):
    self.hidden_reps = hidden_reps
    self.untokenized_words = untokenized_words
  
  def __len__(self):
    return len(self.hidden_reps)
  
  def __getitem__(self, idx):
    return self.hidden_reps[idx], self.untokenized_words[idx]